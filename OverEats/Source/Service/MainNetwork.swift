...
struct MainGet {
    ...
    static func getRestaurantList(completionHandler: @escaping ([Restaurant]) -> Void) {
        ...
        let tag1 = ["일식"]
        let restaurant1 = Restaurant(id: 1, name: "이치라쿠", tag: tag1, score: 4.2, time: 20,account: "테이스티로드에 소개된 라멘 맛집, 가로수길 피에스타 선정 맛집", imageURL: "https://picsum.photos/800/500?random")
        restaurants.append(restaurant1)
        
        let tag2 = ["셀러드", "웰빙", "비건"]
        let restaurant2 = Restaurant(id: 2, name: "알로하 포케 학동점", tag: tag2, score: 0, time: 20,account: "", imageURL: "https://picsum.photos/800/500?random")
